...
import com.example.gnssloggerbutterflying.RinexFilefragments.Constellations.Satellites.BdsSatellite;
import com.example.gnssloggerbutterflying.RinexFilefragments.Constellations.Satellites.EpochMeasurement;
import com.example.gnssloggerbutterflying.RinexFilefragments.Constellations.Satellites.GalileoSatellite;
import com.example.gnssloggerbutterflying.RinexFilefragments.Constellations.Satellites.GlonassSatellite;
import com.example.gnssloggerbutterflying.RinexFilefragments.Constellations.Satellites.GpsSatellite;
import com.example.gnssloggerbutterflying.RinexFilefragments.Constellations.Satellites.QzssSatellite;
...
public class GpsGalileoBdsGlonassQzssConstellation extends Constellation {
    ...
    /**
     * 当结束接收原始数据时，删去已接收到的数据
     */
    public void deleteepochMeasurementList() {
        //this.observedSatellites.clear();
        this.epochMeasurementList.clear();
    }


    private List<EpochMeasurement> epochMeasurementList = new ArrayList<>();

    public List<EpochMeasurement> getEpochMeasurementList() {
        return epochMeasurementList;
    }

    /**
     * 处理一个历元的原始数据，使其变为一个历元数据，，，，注意   这是一个历元，。。。。。。
     */
    public void writeToEpochMessurement() {
        ...
        try {
            ...
            for (SatelliteParameters satelliteParameters : observedSatellites) {
                ...
                if (satelliteParameters.getGpsTime().getGpsTimeString().equals(initalgps.getGpsTimeString())) {
                    ...
                    if (satelliteParameters.getConstellationType() == 6) {
            ...
            }
            {
                ...
                epochMeasurementList.add(epochMeasurement);

                Log.d("gnss", "历元时间:" + epochMeasurement.getEpochTime().getGpsTimeString() + "历元数：" + epochMeasurementList.size());
