...
public class CarrierFreqUtils {
    ...
    public static String getCarrierFrequencyLabel(SatelliteStatus status) {
        if (!SatelliteUtils.isGnssCarrierFrequenciesSupported() || !status.isHasCarrierFrequency()) {
            return CF_UNSUPPORTED;
        }
        float carrierFrequencyMhz = status.getCarrierFrequencyHz()/ 1000000.00f;
        int svid = status.getSvid();

        final float TOLERANCE_MHZ = 1f;
        switch (status.getConstellationType()) {
            case GPS:
            ...
            case GALILEO:
                ...
                break;
            case IRNSS:
                if (fuzzyEquals(carrierFrequencyMhz, 1176.45f, TOLERANCE_MHZ)) {
                    return "L5";
                } else if (fuzzyEquals(carrierFrequencyMhz, 2492.028f, TOLERANCE_MHZ)) {
                    return "S";
                }
                break;
            case SBAS:
